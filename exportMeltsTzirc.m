...
elems={'SiO2';'TiO2';'Al2O3';'Fe2O3';'Cr2O3';'FeO';  'MnO';  'MgO';   'NiO';  'CoO';  'CaO';  'Na2O'; 'K2O'; 'P2O5'; 'H2O_Total';};
...
% Set all H2O 1%
in.H2O_Total=ones(size(in.H2O_Total));
% in.H2O=ones(size(in.H2O))*4;
% in.H2O=ones(size(in.H2O))*nanmean(in.H2O);
% in.H2O(isnan(in.H2O))=nanmean(in.H2O);
...
% test=test &~ (sum(data,2)>100) &~ (sum(data,2)<92);
